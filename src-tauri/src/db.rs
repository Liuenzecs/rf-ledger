...
pub fn resolve_app_data_dir(app: &tauri::AppHandle) -> Result<PathBuf, String> {
  ...
  fs::create_dir_all(&data_dir).map_err(|e| format!("failed to create app data directory {}: {e}", data_dir.display()))?;
  Ok(data_dir)
}
...
pub fn update_transaction(db_path: &Path, id: i64, patch: UpdateTransactionPatch) -> Result<Transaction, String> {
  if id <= 0 {
...
pub fn stats_by_category(db_path: &Path, filters: StatsFilters) -> Result<Vec<CategoryPoint>, String> {
  let conn = open_connection(db_path)?;
  let (from_iso, to_iso) = resolve_stats_range(filters.from, filters.to)?;
...
pub fn maybe_auto_backup(db_path: &Path, app_data_dir: &Path) -> Result<Option<BackupResult>, String> {
...
pub fn export_csv(db_path: &Path, filters: ExportFilters, path: String) -> Result<ExportResult, String> {
  let conn = open_connection(db_path)?;
  ...
  let mut csv_body = String::from("occurred_at,type,amount_cents,category,account,note,created_at,updated_at\n");
...
fn open_connection(db_path: &Path) -> Result<Connection, String> {
  Connection::open(db_path).map_err(|e| format!("failed to open database {}: {e}", db_path.display()))
}
...
fn resolve_stats_range(from: Option<String>, to: Option<String>) -> Result<(String, String), String> {
...
fn validate_export_filters(filters: &ExportFilters) -> Result<(), String> {
  ...
  if let (Some(from), Some(to)) = (&filters.from, &filters.to) {
    let from_dt = DateTime::parse_from_rfc3339(from).map_err(|_| "from must be RFC3339".to_string())?;
    let to_dt = DateTime::parse_from_rfc3339(to).map_err(|_| "to must be RFC3339".to_string())?;
    if from_dt > to_dt {
      return Err("from must be earlier than or equal to to".to_string());
    }
  }
...
fn load_settings(path: &Path) -> Result<LocalSettings, String> {
  ...
  let content = fs::read_to_string(path).map_err(|e| format!("failed to read settings file {}: {e}", path.display()))?;
...
fn save_settings(path: &Path, settings: &LocalSettings) -> Result<(), String> {
  ...
  fs::write(path, content).map_err(|e| format!("failed to write settings file {}: {e}", path.display()))
}
